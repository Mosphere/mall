<template>
    <div>
        ...
        <nav-bread><span slot="first-level">Home</span><span slot="second-level">Order Confirm</span></nav-bread>
...
export default{
    ...
    methods:{
      ...
      createOrder(){
          axios.post('user/createOrder',{'addressId': this.$route.query.addressId,'orderTotal': this.orderTotal}).then((response)=>{
              const res = response.data;
              if(res.status == 200){
                  this.$router.push({name:'OrderSuccess',query:{orderId: res.result.orderId,orderTotal: res.result.orderTotal}});
              }
          }).catch((error)=>{
              console.log(error)
          })
