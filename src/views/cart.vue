<template>
    <div>
      ...
      <nav-bread><span slot="first-level">Home</span><span slot="second-level">cart</span></nav-bread>
                ...
                <li v-for="item in cartList">
                    ...
                    <div class="item-quantity">
                      <div class="select-self select-self-open">
                        <div class="select-self-area">
                          <a class="input-sub" href="javascript:;" @click="editCart('minus',item)">-</a>
                  ...
                  <div class="cart-tab-5">
                    <div class="cart-item-opration">
                      <a href="javascript:;" class="item-edit-btn" @click="deleteConfirm(item)">
          ...
          <div class="cart-foot-wrap">
            <div class="cart-foot-inner">
              <div class="cart-foot-l">
                <div class="item-all-check">
                  <a href="javascipt:;" @click="toggleCheckAll()">
                        <span class="checkbox-btn item-check-btn" v-bind:class="{'check':checkAllStatus}">
              ...
              <div class="cart-foot-r">
                ...
                <div class="btn-wrap">
                  <a class="btn btn--red" v-bind:class="{'btn--dis': checkedCount<1}" @click="checkout()">Checkout</a>
                </div>
              </div>
            </div>
          </div>
      ...
      <modal v-bind:mdShow="mdShow" v-on:close="closeModal()">
        ...
        <div slot="btnGroup">
          <a class="btn btn--m" href="javascript:;" @click="deleteInCart()">是</a>
          <a class="btn btn--m btn-red" href="javascript:;" @click="mdShow = false">否</a>
        </div>
      </modal>
...
export default{
  ...
  methods: {
    ...
    deleteConfirm(item){
      this.mdMsg = '是否从购物车中移除该商品？'
      this.productId = item.productId;
      this.delNum = -item.productNum;
      this.delMoney = item.salePrice*item.productNum;
      this.mdShow = true;
    },
    deleteInCart(){
      this.mdShow = false;
      axios.post('user/cartDelete',{pId: this.productId}).then(response=>{
        const res = response.data;
        if(res.status == 200){
          this.$store.commit("updateCartCount",this.delNum);
          //this.init();
          this.cartList.forEach((item,index)=>{
            if(item.productId == this.productId) this.cartList.splice(index,1);
          })
        }
      }).catch(error=>{
    ...
    editCart(flag,item){
      ...
      axios.post('user/cartEdit',{p_id:item.productId,p_num:item.productNum,checked:item.checked}).then(response=>{
        const res = response.data
        if(res.status == 200){
    ...
    checkout(){
      if(this.checkedCount>0) this.$router.push({name:'Address'});
    },
    toggleCheckAll(){
      let status = !this.checkAllStatus;
      this.cartList.forEach((item)=> {
        item.checked = status;
      });
      axios.post('user/checkAll',{checked: status}).then((response)=>{
        const res = response.data;
        if(res.status == 200){
