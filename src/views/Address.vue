<template>
    <div>
        ...
        <nav-bread>
            <span slot="first-level">Home</span>
            <span slot="second-level">Address</span>
        </nav-bread>
  ...
  <div class="container">
    <div class="checkout-addr">
      ...
      <div class="next-btn-wrap">
        <router-link class="btn btn--m btn--red" v-bind:to="{name: 'OrderConfirm',query:{'addressId': selectedAddressId}}">Next</router-link>
      </div>
    </div>
  </div>
</div>
      <modal v-bind:mdShow="mdShow" v-on:close="closeModal()">
        <p slot="message">是否删除该收货地址</p>
        <div slot="btnGroup">
          <a class="btn btn--m" href="javascript:;" @click="deleteAddress()">是</a>
          <a class="btn btn--m btn-red" href="javascript:;" @click="mdShow = false">否</a>
        </div>
      </modal>
      <modal v-bind:mdShow="deleteAlert" v-on:close="closeModal()">
        <p slot="message">地址列表至少需要一条数据</p>
        <div slot="btnGroup">
          <a class="btn btn--m btn-red" href="javascript:;" @click="deleteAlert = false">好的</a>
        </div>
      </modal>
...
export default{
    ...
    methods:{
      getAddressList(){
        axios.get('user/getAddressList').then((response)=>{
      ...
      setDefaultAddress(addressId){
        axios.post('user/setDefaultAddress',{"addressId": addressId}).then((response)=>{
          const res = response.data;
          if(res.status == 200){
            this.getAddressList();
          }
      ...
      deleteAddress(){
        axios.post('user/deleteAddress',{"addressId": this.delAddressId}).then((response)=>{
          const res = response.data;
          if(res.status == 200){
            this.mdShow = false;
            this.getAddressList();
      ...
      addAddress(){
        ...
        axios.post('user/addAddress',addressData).then((response)=>{
          const res = response.data;
          if(res.status == 200){
            this.mdShow = false;
            this.getAddressList();
